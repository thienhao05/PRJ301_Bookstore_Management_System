...
public class MainController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            } else if ("checkout".equals(action) || "history".equals(action)
                    || "dashboard".equals(action) || "manageOrders".equals(action)
                    || "manageOrderDetail".equals(action) || "updateStatus".equals(action)
                    || "update_status".equals(action) || "myOrders".equals(action)
                    || "myOrderDetail".equals(action) || "updateOrderStatus".equals(action)) {
                url = ORDER;
    ...
    /**
     * Nếu action khớp với actionName thì set attribute "action" = mappedValue
     * để các Controller con đọc được đúng action.
     */
