...
public class UserController extends HttpServlet {
        ...
        try {
            switch (action) {
                ...
                case "profile":
                    ...
                    break;
                case "updateProfile": // ✅ THÊM MỚI
                    doUpdateProfile(request, response, session);
                    break;
                case "forgotPassword":
    ...
    private void doUpdateProfile(HttpServletRequest request, HttpServletResponse response,
        ...
        if (fullName == null || fullName.trim().isEmpty()
            ...
            session.setAttribute("MSG_ERROR", "Họ tên và email không được để trống!");
            response.sendRedirect("MainController?action=profile");
            return;
        }

        // Cập nhật thông tin — giữ nguyên password và roleId
        loginUser.setUsername(fullName.trim());
        loginUser.setEmail(email.trim());
        loginUser.setPhone(phone != null ? phone.trim() : "");

        if (userDAO.update(loginUser)) {
            // ✅ Cập nhật lại session để header hiển thị tên mới ngay
            session.setAttribute("LOGIN_USER", loginUser);
            session.setAttribute("MSG_SUCCESS", "Cập nhật hồ sơ thành công!");
        } else {
            session.setAttribute("MSG_ERROR", "Cập nhật thất bại, vui lòng thử lại!");
        }
